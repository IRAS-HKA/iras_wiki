## HiWi Anstellung
1. Dokument Vorlage in [Excel](https://hskarlsruhede.sharepoint.com/:x:/s/RobolabLeitung2/EXcnohbKXR1KpBJ_eEXXnr0Bmd204iwSaiTK2Jnx5g3Gvg?e=1uvhR4) [Teams Ordner](https://hskarlsruhede.sharepoint.com/:f:/s/RobolabLeitung2/EkCm9s6wMTNJiSGUB8ibxlABhReuVROPwwAbBZU6741FMw?e=uroR0d)
2. Projekt: Robot-Hub-Transfer
	CAR-Nr.: 1039
	SAP/PSP Element: 8056 20000 77501
3. An Fr. Förderer schicken sabine.foerderer@h-ka.de
4. Vertrag kommt von Personal (Kloiber-Hasic) direkt per Post
5. (Interreg) Aufgabenbeschreibung ausfüllen und an Fr. Förderer

## Raumbelegungssystem
1. Bei [Raumzeit](https://raumzeit.hka-iwi.de/) mit RZ-Kürzel und RZ-Passwort anmelden.
2. Information := "Räume"
3. Fakultät / Einrichtung := "Institut für Robotik und Autonome Systeme"
4. Auswahl Raum und Datum zum Anzeigen der Raumbelegung
5. Mit Doppelklick auf den Kalender öffnet sich das Fenster für Buchungsanfragen. Pflichtfelder unter "Name und Kontakt" ausfüllen und Angaben unter "Zeiten und Räume" anpassen. Speichern. 

## Zeiterfassung (elektronisch) mit SmartTime
1. [SmartTime](https://ze01.hs-karlsruhe.de/SmartTime/) öffnen
2. Mit RZ-Kürzel und RZ-Passwort anmelden
3. Unter "Buchen" Arbeitsbeginn und Ende erfassen.

## Urlaubsantrag
1. Antrag in [SmartTime](https://ze01.hs-karlsruhe.de/SmartTime/#/security/loginInput.action?h=1633343827654) stellen
2. Projektleiter (Christian oder Björn) muss bestätigen oder es wird nach 1 Woche automatisch genehmigt
3. Wenn Urlaub bestätigt ist, auch in [W-Plan](https://wplan.h-ka-w.de/wplan/benutzer/login/?next=/wplan/) eintragen. (für Einsicht von Kollegen)
	- Reiter "Fakultät" > "Mein Urlaub" > Klick auf "Neuer Urlaub"

## CAR Bestellungen
- Ab 500 € ein Angebot
- Ab 1000 € drei Angebote
1. Links oder Angebote an Frau Förderer schicken
2. Sie schaut ob Geld da ist und leitet an Beschaffung weiter

## Masterprojekt Ausschreibung
1. Erstellen mit Template [RoboTrainer Teams](https://hskarlsruhede.sharepoint.com/:w:/t/RoboTrainer/ERY-t0MXKWBEg4ZSezCBspABdvGdAnL_NDWCRRD5rMcs3g?e=Dlkpft)
	- Projekt Code und Dateiname müssen übereinstimmen
	- Format: YYws/ss_2LetterLastName_ShortProjectTitle
	- Example: 24ws_HE_RoboTrainer_Ganganalyse
2. An Robert Weiß per Mail schicken robert.weiss@h-ka.de
> 	Hallo Herr Weiß,
> 	im Anhang finden Sie eine neue Ausschreibung für ein Forschungs und Entwicklungsprojekt für den Masterstudiengang RKIM.
> 	Bitte laden Sie es an entsprechender Stelle in ILIAS hoch.
3. In ILIAS in diesen Ordner hochladen [ILIAS](https://ilias.h-ka.de/goto.php?target=cat_870471&client_id=HSKA).
4. Studis finden und zusagen
	- Studis müssen auf Ausschreibung unterschreiben
	- Betreuer muss unterschreiben
5. Studis laden die unterschriebene Ausschreibung selber in ILIAS hoch und melden sich zum entsprechenden Projekt Code an.

## Dienstreiseantrag
Alle Dokumente der Personalabteilung [HKA Intern](https://www.h-ka.de/intern/mitarbeitende/organisieren/dokumente-formulare/formulare-vorlagen) (nur erreichbar im Hochschulnetz)
1. [Antrag zur Genehmigung der Dienstreise](https://www.h-ka.de/securedl/sdl-eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..aJNI4zrKUypz0OBpLnxdD37Pw4V-ReP1hJLDGfGOMTk/HKA_VW-PE_Formular_Genehmigung_DienstFortbildungsreise_Februar_2022_elektronisch_ausfuellbar.pdf) herunterladen und ausfüllen
	1. Kostenvoranschlag eintragen (wichtig)
	2. Unterschrift reicht digital
	3. Reise genehmigt unterschreiben von Christian/Björn
   	4. Immer Scan von Dienstreiseantrag machen (Hauspost = Schwarzes Loch)
	5. Dann an per Hauspost an CAR Fr. Förderer
2. EU Ausland
   	2. Zuerst Antrag an International Office schicken, danach an CAR Fr. Förderer
   	3. A1 Antrag Fragebogen ausfüllen (42103a) [LBV](https://lbv.landbw.de/vordrucke#vordrucknummer=42103)
   	4. A1 Antrag und genehmigte Dienstreise (es reicht die Unterschrift von Christian/Björn) an CAR Personal schicken (Email reicht)
3. Nicht-EU Ausland
   	1. Erst Antrag an International Office schicken, danach an CAR Fr. Förderer
   	2. DVKA Antrag ausfüllen (Länderspezifisch) [DVKA](https://www.dvka.de/de/arbeitgeber-erwerbstaetige/antraege-finden/entsendung-ins-ausland/entsendung/)
   	3. Antrag an CAR Personal (Email reicht)
4. [Abrechnung der Dienstreise](https://www.h-ka.de/securedl/sdl-eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..Q8wu00LlJGnhy0QtqgA2JJFdtDDnjrNxLm9cELx0AOQ/HKA_VW-PE_Formular_Abrechnung_DienstFortbildungsreise_2022_geschuetzt.doc)
	- Dienstreiseabrechnung
	- Alle Rechnungen
	- Originaler Dienstreiseantrag
	- Per Hauspost an Personal / Silke Wieland
	- Falls am Wochenende gereist oder konferiert wurde den genehmigten Dienstreiseantrag an Zeiterfassung schicken um als Urlaubstage zurückzuholen.

## Dienstreise buchen
Reisebüro der Hochschule:  
> 	**City Air Terminal Reisebüro (CAT)**  
> 	Lufthansa City Center  
> 	Terminal 1  
> 	70629 Stuttgart (Flughafen)  
> 	Telefon: 0711 - 948 4445  
> 	E-Mail: Team-BW@cityairterminal.de  
> 	Öffnungszeiten:  
> 	Montag bis Freitag: 08:00 – 18:00 Uhr  
> 	Sa/So und Feiertag: 09:00 – 14:00 Uhr  
> 	Außerhalb der Öffnungszeiten erreichen Sie den 24 Std. Service unter Tel.: 030 – 69802 111  
- **Die Kundennummer Hochschule Karlsruhe lautet bei CAT Reisebüro: 100180**
- (bitte auch Geburtsdatum bei Anfrage nennen --  bitte bevorzugt per E-Mail anfragen)
- Zu Vereinfachung der anschließenden Bezahlung durch die CAR-FIN setzen Sie mich doch bitte in CC bei Ihrem GBT-Schriftverkehr michael.christ@h-ka.de
- Verbunden mit Urlaubsreise:
	 - Im Dienstreiseantrag ankreuzen "Die Reise wird mit einer Privat-/Urlaubsreise verbunden"
	 - wenn Flug erstattet werden soll, müssen entsprechende Vergleichsangebote eingeholt werden:
		- ein Angebot für den regulären Rückflugtag direkt nach Konferenz
		- ein Angebot für den Rückflugtag nach angehängtem Privaturlaub
		- die Differenz muss selbst bezahlt werden.
		- Für den Zeitraum des Privaturlaubs anschließend oder davor muss Hotel selbst bezahlt werden.
- Sie haben die Möglichkeiten, für größere Reisen einen Vorschuss zu beantragen. Diesen können Sie formlos zusammen mit einer Kopie des genehmigten Antrages, einer Übersicht der zu erwartenden Kosten und unter Angabe der Bankverbindung bei der Personalabteilung einreichen.

## Neue Hauspostumschläge beantragen
- Formlose Email an Peter Herlan (Poststelle) peter.herlan@h-ka.de

## Promotionsvereinbarung mit der HKA als Kooperative Einrichtung
- [Formular](https://www.h-ka.de/fileadmin/Hochschule_Karlsruhe_HKA/Satzungen_Ordnungen/HKA_ZH_Betreuungsvereinbarung_Promotionsvorhaben_V1_4_2021-12-13.pdf) ausfüllen.
- Formlose Email mit Annahme bei Partneruniversität und Betreuungsvereinbarung an Verena Tomczyk. 
